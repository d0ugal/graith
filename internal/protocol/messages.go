...
// Version 3 makes terminal-owned attach the CLI default and removes the
// experimental attach request key. The major bump is intentional: older clients
// must not connect and silently revive the removed raw attach fallback.
const Version = "3.0"

const (
	TerminalOwnedAttachSeedNotReadyMessage = "terminal-owned attach seed is not ready; retry shortly"
	TerminalOwnedAttachUnsupportedMessage  = "session does not provide terminal-owned attach output"
	TerminalOwnedAttachRawResponseMessage  = "terminal-owned attach returned raw attached response"
)
...
type AttachMsg struct {
	...
	ReadOnly bool `json:"read_only,omitempty"`
	// TerminalOwned requests the terminal-owned attach handshake. The daemon
	// replies with the "terminal_owned_attached" control type when it can
	// seed the client with an atomic current-screen snapshot before live data
	// frames resume; otherwise it returns an error.
	TerminalOwned bool `json:"terminal_owned,omitempty"`
}

type TerminalOwnedAttachSeedMsg struct {
