...
func TestValidateTriggers_Invalid(t *testing.T) {
	cases := []struct {
		...
		{"scratch repo conflict", schedTrigger("scunner", ScheduleConfig{Cron: "@daily"}, ActionConfig{Type: ActionSession, NoRepo: true, Repo: "/tmp/croft"}), true, "incompatible with action.repo"},
		{"scratch watch conflict", watchTrigger("scunner", WatchConfig{Role: "impl"}, ActionConfig{Type: ActionSession, NoRepo: true}), true, "incompatible with watch/completion"},
		{"missing prompt file", schedTrigger("scunner", ScheduleConfig{Cron: "@daily"}, ActionConfig{Type: ActionSession, PromptFile: "/dreich/missing.md"}), true, "prompt_file"},
