...
func (sm *SessionManager) Delete(id string) error {
	...
	if hasPTY {
		...
		if err := sm.teardownLiveDriver(ptySess); err != nil {
			// teardownLiveDriver could not confirm the process exited (even SIGKILL
			// left Done open), so it deliberately left the driver owned by the launch
			// watcher rather than Close-ing it. Committing the delete now would orphan
			// a live agent and tear down its worktree with no way to reach the
			// process. Abort and keep the session recoverable: restore the driver into
			// sm.sessions and its PID into state so a retry can finish the job, and
			// drop the recovery tombstone only once that retry state is durable — the
			// delete did not commit and must not auto-resume against a live process
			// (issue #1326).
			...
			// marker — it is the sole record of the still-live PID — and surface the
			// error.
			var rmErr error
			if saveErr == nil {
				rmErr = sm.removeTombstone(id)
		...
		if _, err := sm.killVerifiedProcess(orphanPID, orphanStartTime); err != nil {
			...
			// durable removal itself failed: the process may still be alive and the
			// marker is the only durable record of it (issue #1326).
			var rmErr error
			if saveErr == nil {
				rmErr = sm.removeTombstone(id)
			}
...
func (sm *SessionManager) tombstoneBeforeBulkTeardown(spec teardownSpec, name string, pid int, pidStartTime int64) error {
	if err := sm.writeTombstone(tombstone{
		...
		// the single-delete abort). Surface a removal failure too — the caller must
		// not report a cleanly restored session while a marker lingers.
...
// Running, still reachable). Mirrors the single-delete pre-teardown abort (issue
// #1326). Because the process is untouched, the caller must not kick the restored
// client.
...
// keepTombstone is true when the session is kept for retry but its restored
...
func (sm *SessionManager) teardownBulkDeleteDriver(id, name string, driver SessionDriver, pid int, pidStartTime int64, initiator string) (liveFailed, killFailed, keepTombstone bool, err error) {
	if driver != nil {
		...
		if e := sm.teardownLiveDriver(driver); e != nil {
...
// killBulkDeleteOrphan kills a bulk-delete snapshot's orphaned process group — a
// session recorded with a live PID but no live driver (e.g. spawned into the tree
// after a daemon crash) — and, on failure, keeps the session for retry: errored,
// with its PID/identity restored. It returns a non-nil retryErr when the kill
// failed so the caller keeps the session out of the teardown/removal set and
...
// termination during a bulk delete so a later retry can finish the job. The
// driver is put back into sm.sessions and its PID restored into state, and the
...
func (sm *SessionManager) DeleteWithChildren(id string, excludeRoot bool) ([]string, error) {
	...
	for _, s := range snaps {
		...
		live, killed, keepTombstone, err := sm.teardownBulkDeleteDriver(s.id, s.name, s.ptySess, s.pid, s.pidStartTime, "delete-children")
		if live {
		...
		if err != nil {
			...
			// Kept for retry: the restore path re-armed its PID, so drop the recovery
			// marker — but only if that retry state was durably saved. If the save
			// failed (keepTombstone), the marker is the sole record of the still-live
			// PID and must be retained. A durable removal failure is itself surfaced,
			// since a lingering marker would resurrect the delete against the kept
			// session (#1326).
			if !keepTombstone {
	...
	for sweep := 0; sweep < maxSweepRounds; sweep++ {
		...
		for _, s := range lateSnaps {
			...
			live, killed, keepTombstone, err := sm.teardownBulkDeleteDriver(s.id, s.name, s.ptySess, s.pid, s.pidStartTime, "delete-children-sweep")
			if live {
				liveFailed[s.id] = true
			}
...
// procKill signals a process — or, for a negative pid, a process group. It is a
// package var only so tests can simulate signal outcomes real processes cannot
// produce on demand: an EPERM probe, a failed SIGKILL, or a group that dies only
// after several probes (issue #1326). Production always uses syscall.Kill.
var procKill = syscall.Kill

// killProcessGroup sends SIGTERM to the process group led by pid, waits up to
// grace for the group to exit, then escalates to SIGKILL and waits a further
// bounded window for the group to actually disappear. It returns nil ONLY when
// the group is confirmed gone — a liveness probe (signal 0) that fails with
// ESRCH. A non-positive grace falls back to the built-in default (the daemon
// passes the [lifecycle] process_kill_grace policy). The escalation ORDER
// (SIGTERM → SIGKILL) is fixed.
//
// The SIGKILL error is checked and death is confirmed by probing, so a failed
// kill or a process we lack permission to signal (EPERM) is never mistaken for a
// successful teardown. Reporting a live process as dead here would let a caller
// tear down its worktree and drop its state while the agent is still running
// (issue #1326).
func killProcessGroup(pid int, grace time.Duration) error {
	...
	pgid := -pid
	if err := procKill(pgid, syscall.SIGTERM); err != nil {
	...
	// Wait up to grace for a clean SIGTERM exit, polling for confirmed absence
	// rather than treating any probe error as death.
	if processGroupGone(pgid, grace) {
		return nil
	}

	if err := procKill(pgid, syscall.SIGKILL); err != nil {
		if errors.Is(err, syscall.ESRCH) {
			return nil
		}
	...
	// SIGKILL cannot be caught, but reaping is asynchronous. Give the group a
	// bounded window to actually disappear and confirm it before reporting
	// success; a group still present at the deadline may hold a live process
	// (uninterruptible sleep, unreaped leader, or one we cannot signal), so fail
	// closed rather than proceed to tear down its worktree (issue #1326).
	if processGroupGone(pgid, grace) {
		return nil
	}

	return fmt.Errorf("process group %d still present %s after SIGKILL", pid, grace)
}

// processGroupGone polls the process group led by pgid (a negative pid) until a
// liveness probe proves it gone or the window expires. Only ESRCH — no process
// in the group — counts as gone. EPERM (the group exists but we lack permission
// to signal it) or any other probe error means a process is still there and must
// NOT be read as absence (issue #1326).
func processGroupGone(pgid int, window time.Duration) bool {
	if errors.Is(procKill(pgid, 0), syscall.ESRCH) {
		return true
	}
	...
	for {
		select {
		case <-deadline:
			// Final probe so a group that exited right at the deadline is not
			// reported as still present.
			return errors.Is(procKill(pgid, 0), syscall.ESRCH)
		case <-tick.C:
			if errors.Is(procKill(pgid, 0), syscall.ESRCH) {
				return true
			}
...
// teardownLiveDriver removes a live session driver with a bounded escalation:
// SIGTERM, wait one grace, SIGKILL, wait one further grace, and only then Close
// the driver handles. It returns an error if even SIGKILL does not confirm exit
// within the grace window — in that case the driver is deliberately NOT Closed
// (the launch-time watcher keeps ownership) so this operation cannot wedge, and
// the caller must keep the session for retry rather than tear down its worktree
// while the process may still be alive (issue #1326).
//
...
func (sm *SessionManager) teardownLiveDriver(driver SessionDriver) error {
	...
	// Once SIGKILL is requested give reaping/output drains their own bounded
	// completion window before deciding the process would not die.
	if waitDriverDone(driver, grace) {
		driver.Close()
		return nil
	}

	timeoutErr := fmt.Errorf("driver did not exit within %s after SIGKILL", grace)
	if termErr != nil {
		timeoutErr = errors.Join(timeoutErr, fmt.Errorf("SIGTERM: %w", termErr))
	}

	if forceErr != nil {
		timeoutErr = errors.Join(timeoutErr, fmt.Errorf("SIGKILL: %w", forceErr))
	}
