...
// assertHumanTokenRejected writes an existing token file via setup, then
// asserts loadOrCreateHumanToken fails closed and leaves humanToken empty.
// `what` names the rejected condition for the failure messages.
func assertHumanTokenRejected(t *testing.T, what string, setup func(t *testing.T, tokenPath string)) {
	t.Helper()

	tokenPath := filepath.Join(t.TempDir(), "human.token")
	setup(t, tokenPath)
	...
	if sm.humanToken != "" {
		t.Fatalf("human token populated despite %s", what)
	}
}

func TestLoadHumanTokenRejectsPermissiveMode(t *testing.T) {
	assertHumanTokenRejected(t, "a group/other-readable token", func(t *testing.T, tokenPath string) {
...
func TestLoadHumanTokenRejectsSymlink(t *testing.T) {
	assertHumanTokenRejected(t, "a symlinked token", func(t *testing.T, tokenPath string) {
		realPath := filepath.Join(filepath.Dir(tokenPath), "real.token")
...
func TestLoadHumanTokenRejectsEmptyContent(t *testing.T) {
	assertHumanTokenRejected(t, "a whitespace-only token file", func(t *testing.T, tokenPath string) {
