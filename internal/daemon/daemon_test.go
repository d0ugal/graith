...
func TestReloadConfigRejectsUnsafeTimingPolicies(t *testing.T) {
	tests := []struct {
		...
		// Top-level PR-watch poll cadences and the anti-flood debounce (#1304).
		{"PR poll_pending", "[pr_watch]\npoll_pending = \"0s\"\n", "pr_watch.poll_pending"},
		{"PR poll_terminal", "[pr_watch]\npoll_terminal = \"-1s\"\n", "pr_watch.poll_terminal"},
		{"PR poll_merged", "[pr_watch]\npoll_merged = \"0s\"\n", "pr_watch.poll_merged"},
		{"PR debounce", "[pr_watch]\ndebounce = \"-1s\"\n", "pr_watch.debounce"},
		// Human approval timeout composed into the operation budget (#1251).
		{"approvals human timeout", "[approvals]\ntimeout = \"0s\"\n", "approvals] timeout"},
...
func TestDaemonHandshakeReportsAcceptedApprovalBound(t *testing.T) {
	...
	if got := sm.InstanceID(); got == "" {
		t.Error("handshake identity: instance ID must be non-empty")
	}
}
