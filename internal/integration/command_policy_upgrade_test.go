...
func TestCommandPolicyRemovalUpgradeFromExactBaseline(t *testing.T) {
	...
	daemonCmd := exec.Command(oldBinary, "--config", configPath, "daemon", "start") //nolint:gosec // Exact test-owned binary and arguments.
	daemonCmd.Env = env
	...
	downgrade := exec.CommandContext(downgradeCtx, oldBinary, "--config", configPath, "daemon", "start") //nolint:gosec // Exact test-owned baseline binary.
	downgrade.Env = env
...
func buildGraithBinary(t *testing.T, sourceDir, output string) {
	t.Helper()
	cmd := exec.Command("go", "build", "-o", output, "./cmd/graith") //nolint:gosec // Fixed Go tool invocation in a test-owned source tree.
	cmd.Dir = sourceDir
...
func runFixtureCLI(t *testing.T, env []string, binary string, args ...string) []byte {
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, binary, args...) //nolint:gosec // Test-owned exact/current Graith binary.
	cmd.Env = env
	output, err := cmd.CombinedOutput()
	if err != nil {
