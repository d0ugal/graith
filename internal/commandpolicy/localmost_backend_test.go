...
func TestLocalmostEvaluationTimeout(t *testing.T) {
	script := writePolicyScript(t, "exec sleep 2\n")
	start := time.Now()
	_, err := (localmostBackend{}).Evaluate(context.Background(), Request{
		ToolName: "run_shell_command", ToolInput: `{"command":"echo strath"}`,
	}, Config{Command: script, ExecTimeout: 50 * time.Millisecond})
	if err == nil || !strings.Contains(err.Error(), "timed out") {
		t.Fatalf("Evaluate error = %v, want timeout", err)
	}
	if elapsed := time.Since(start); elapsed > time.Second {
		t.Fatalf("bounded evaluation took %v", elapsed)
	}
}

func TestLocalmostEvaluationTimeoutClosesDescendantPipes(t *testing.T) {
	script := writePolicyScript(t, "sleep 10 &\nwait\n")
	start := time.Now()
	_, err := (localmostBackend{}).Evaluate(context.Background(), Request{
		ToolName: "Bash", ToolInput: `{"command":"echo strath"}`,
	}, Config{Command: script, ExecTimeout: 50 * time.Millisecond})
	if err == nil || !strings.Contains(err.Error(), "timed out") {
		t.Fatalf("Evaluate error = %v, want timeout", err)
	}
	if elapsed := time.Since(start); elapsed > time.Second {
		t.Fatalf("descendant-held pipes exceeded the backend bound: %v", elapsed)
	}
}
