...
func TestLibghosttyLinuxArtifactsRemainTrustedAndLockComplete(t *testing.T) {
	...
	assertContains(t, nativePublish, "verify_relocatable_pkg_config()")
	assertContains(t, nativePublish, `grep -Fqx "prefix=\${pcfiledir}/.." "$pc"`)
	assertContains(t, nativePublish, "pkg-config prefix is not relocatable")
	assertContains(t, nativePublish, `read -r -a cflag_tokens <<< "$cflags"`)
	assertContains(t, nativePublish, `include_flag="${cflag_tokens[0]:-}"`)
	assertContains(t, nativePublish, `"${#cflag_tokens[@]}" -ne 2`)
	assertContains(t, nativePublish, `"${cflag_tokens[1]:-}" != "-DGHOSTTY_STATIC"`)
	assertContains(t, nativePublish, `! include_path="$(realpath "$include_path")`)
	assertContains(t, nativePublish, `! expected_include="$(realpath "$expected_include")`)
	assertContains(t, nativePublish, "pkg-config cflags use unexpected flags")
	assertContains(t, nativePublish, `read -r -a lib_tokens <<< "$libs"`)
	assertContains(t, nativePublish, `lib_flag="${lib_tokens[0]:-}"`)
	assertContains(t, nativePublish, `"${#lib_tokens[@]}" -ne 2`)
	assertContains(t, nativePublish, `"${lib_tokens[1]:-}" != "-lghostty-vt"`)
	assertContains(t, nativePublish, `! lib_path="$(realpath "$lib_path")`)
	assertContains(t, nativePublish, `! expected_lib="$(realpath "$expected_lib")`)
	assertContains(t, nativePublish, "pkg-config libs use unexpected flags")
	assertNotRegexp(t, nativePublish, `test[[:space:]]+"\$\([^)]*pkg-config --(cflags|libs) libghostty-vt-static[^)]*\)"`)
	assertNotContains(t, nativePublish, `test "$(PKG_CONFIG_PATH="$RUNNER_TEMP/bundle/pkgconfig" pkg-config --cflags libghostty-vt-static)"`)
	assertNotContains(t, nativePublish, `test "$(PKG_CONFIG_PATH="$RUNNER_TEMP/bundle/pkgconfig" pkg-config --libs libghostty-vt-static)"`)
...
func TestLibghosttyNativeArtifactWorkflowPublishesBeforeGeneration(t *testing.T) {
	...
	assertContains(t, linuxBuild.Run, "verify_relocatable_pkg_config()")
	assertContains(t, linuxBuild.Run, `grep -Fqx "prefix=\${pcfiledir}/.." "$pc"`)
	assertContains(t, linuxBuild.Run, "pkg-config prefix is not relocatable")
	assertContains(t, linuxBuild.Run, `read -r -a cflag_tokens <<< "$cflags"`)
	assertContains(t, linuxBuild.Run, `include_flag="${cflag_tokens[0]:-}"`)
	assertContains(t, linuxBuild.Run, `"${#cflag_tokens[@]}" -ne 2`)
	assertContains(t, linuxBuild.Run, `"${cflag_tokens[1]:-}" != "-DGHOSTTY_STATIC"`)
	assertContains(t, linuxBuild.Run, `! include_path="$(realpath "$include_path")`)
	assertContains(t, linuxBuild.Run, `! expected_include="$(realpath "$expected_include")`)
	assertContains(t, linuxBuild.Run, "pkg-config cflags use unexpected flags")
	assertContains(t, linuxBuild.Run, `read -r -a lib_tokens <<< "$libs"`)
	assertContains(t, linuxBuild.Run, `lib_flag="${lib_tokens[0]:-}"`)
	assertContains(t, linuxBuild.Run, `"${#lib_tokens[@]}" -ne 2`)
	assertContains(t, linuxBuild.Run, `"${lib_tokens[1]:-}" != "-lghostty-vt"`)
	assertContains(t, linuxBuild.Run, `! lib_path="$(realpath "$lib_path")`)
	assertContains(t, linuxBuild.Run, `! expected_lib="$(realpath "$expected_lib")`)
	assertContains(t, linuxBuild.Run, "pkg-config libs use unexpected flags")
	assertNotRegexp(t, linuxBuild.Run, `test[[:space:]]+"\$\([^)]*pkg-config --(cflags|libs) libghostty-vt-static[^)]*\)"`)
	assertNotContains(t, linuxBuild.Run, `test "$(PKG_CONFIG_PATH="$RUNNER_TEMP/bundle/pkgconfig" pkg-config --cflags libghostty-vt-static)"`)
	assertNotContains(t, linuxBuild.Run, `test "$(PKG_CONFIG_PATH="$RUNNER_TEMP/bundle/pkgconfig" pkg-config --libs libghostty-vt-static)"`)
