...
func TestCheckUnknownAndExpiredManifestData(t *testing.T) {
	...
	if _, err := Check(m, []Package{{ImportPath: "example.com/braw/a"}}, now); !strings.Contains(err.Error(), "expired") {
		t.Fatalf("err=%v", err)
	}
	...
	if _, err := Check(m, []Package{{ImportPath: "example.com/braw/a"}}, now); !strings.Contains(err.Error(), "unknown imported") {
		t.Fatalf("err=%v", err)
	}
	...
	if _, err := Check(m, []Package{{ImportPath: "example.com/braw/a"}, {ImportPath: "example.com/braw/b"}}, now); !strings.Contains(err.Error(), "duplicate exception edge") {
		t.Fatalf("err=%v", err)
	}
}
...
func TestDiscoveryVariantsCoverSupportedBuilds(t *testing.T) {
	...
	for _, variant := range variants {
		key := variant.goos + "/" + variant.goarch + ":" + strings.Join(variant.tags, ",") + ":" + strconv.FormatBool(variant.cgo)
		seen[key] = true
	}
