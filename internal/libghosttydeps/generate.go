...
func writeGeneratedMetadata(root string, lock Lock) error {
	...
	if err := os.WriteFile(filepath.Join(root, SPDXFilename), spdx, 0o644); err != nil {
	...
	if err := os.WriteFile(noticesPath, []byte(updated), 0o644); err != nil {
...
func updateSwiftPackage(root string, lock Lock) error {
	...
	updated := urlPattern.ReplaceAllString(string(data), `url: "`+lock.Ghostty.AppleArtifact.URL+`"`)
	updated = checksumPattern.ReplaceAllString(updated, `checksum: "`+lock.Ghostty.AppleArtifact.SHA256+`"`)
