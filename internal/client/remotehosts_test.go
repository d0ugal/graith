...
func runContendingRemoteHostHelpers(t *testing.T, path string, actions ...string) []string {
	...
	err := withRemoteHostStoreLock(path, func(_ *RemoteHostStore) error {
		for i, action := range actions {
			...
			helper.cmd = exec.Command(os.Args[0], "-test.run=^TestRemoteHostStoreCrossProcessSerialization$") //nolint:gosec // G702: deliberately re-executes this fixed test binary.
