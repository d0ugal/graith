...
When an agent hits a confusing "permission denied", you're really asking one of
two questions — and `gr sandbox` has a subcommand for each:
...
backend: a future backend with an oracle joins `explain`, and a future OS audit
log joins `watch`.
...
graith would generate from your config, asks the backend's policy oracle, then
...
`environment.allow_vars` allowlist), so it's a faithful preview of what a real
session would enforce. Note that a read-only `read_dirs`/`read_files` grant at
or under `/tmp` or `$TMPDIR` is rejected up front (see the
...
which paths and operations were blocked. It's the practical way to debug a
...
would tail forever with no way to stop. Pass `--follow` (`-f`) to force a live
tail there.
...
- Live (the default on a terminal), membership is re-checked against a
  ...
  more reliably than `--recent`, but it isn't perfect: a child denied and exited
  before the snapshot sees it can't be attributed (the macOS log carries no
  session identity), and over a long watch a recycled PID could be
  misattributed.
- With `--recent`, graith can only match against the process tree as it exists
  **now**, so denials from children that have since exited are missing and a
  recycled PID can be misattributed. `gr sandbox watch <session> --recent`
...
> normal shell, not from a sandboxed agent session.
