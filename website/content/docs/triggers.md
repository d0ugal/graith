...
definition or source change does not silently adopt an incompatible old reactor:
the daemon attempts to stop and soft-delete the old trigger-owned reactor, and
the next eligible event creates a fresh one. If retirement cannot be completed,
the reactor remains excluded from reuse and the daemon logs the failure.
Pausing/removing a trigger leaves its reactor recoverable but does not cause a
later re-added definition to adopt it unless the definition still matches. A
soft-deleted reactor is likewise never adopted.
...
while an inbox enqueue failure is recorded as a trigger error and is not retried
automatically. Resume or PTY-injection failures are logged as delivery failures
by the background session machinery. Only stable IDs and URLs are placed in the prompt; raw
