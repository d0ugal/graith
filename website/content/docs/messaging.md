...
`conversation_max_limit`. When only the maximum is lowered, the inherited page
size is derived down to the new maximum; an explicit contradiction is rejected.
The conversation paging bounds and the jail cap are read per request, so a
