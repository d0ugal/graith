...
| `session-navigator-preview.yml` | deterministic TUI screenshot preview | `trigger` | migrated | file-list or classifier failure renders the synthetic TUI screenshots |
...
`internal/ciworkflow` also select the dev-release, docs-preview, and TUI
preview gates, because those workflows now rely on the shared
classifier.
...
- docs-preview, TUI preview, renovate retry, libghostty native,
