...
`log_lines` is the shared default when a `--lines`/`-n` count is omitted: `gr logs` and attach scroll mode send `0`, so the daemon applies its current value per request — reconnecting attach sessions and graphical clients' log peeks pick up the same server-side default after a reload. Sessions created with `--experimental-attach` also use this value as their initial daemon-owned terminal history depth; that retention is fixed when the PTY backend is created and is clamped by the native terminal model to at most 2,000 rows and about 256 Ki cells. `last_message_runes` and `inbox_preview_bytes` cap what's shown; the full message stays available via `gr msg inbox --all`. Byte caps never split a multi-byte character mid-rune, and `last_message_runes` is counted in whole runes.
