...
enum AgentPreference {
    ...
    /// explicit value only when this host offers it; otherwise `""` ("follow this
    /// host's daemon default").
    ///
    /// The preference is stored globally, so this is deliberately read-only: a
    /// host that does not offer the agent shows the daemon-default row but must
