...
struct SessionRow: View {
    ...
    var isInSplitPane: Bool {
        store.isSplit && store.splitSessionID == session.id
    }
    ...
    var rowBackground: Color {
        if isSelected && store.focusedPane == .primary && store.isSplit {
            return Theme.surface0
        }
        if isInSplitPane && store.focusedPane == .secondary && store.isSplit {
            return Theme.surface0
        }
        if isHighlighted {
