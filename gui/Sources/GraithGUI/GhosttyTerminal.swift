...
final class GhosttyTerminalState {
    ...
    init(cols: UInt16, rows: UInt16, maxScrollback: Int = 10000) {
        self.cols = cols
        self.rows = rows
    ...
    func resize(cols: UInt16, rows: UInt16, cellWidth: UInt32 = 8, cellHeight: UInt32 = 16) {
        guard let terminal else { return }
    ...
    func getColors() -> GhosttyRenderStateColors {
    ...
    func getCursorInfo() -> (visible: Bool, x: UInt16, y: UInt16, style: GhosttyRenderStateCursorVisualStyle)? {
    ...
    struct CellInfo {
        ...
        let fgColor: GhosttyColorRgb?
        let bgColor: GhosttyColorRgb?
        let bold: Bool
        let italic: Bool
        let underline: Bool
    ...
    func iterateRows(_ callback: (_ row: Int, _ dirty: Bool, _ cells: [CellInfo]) -> Void) {
    ...
    func encodeKey(action: GhosttyKeyAction, key: GhosttyKey, mods: GhosttyMods, text: String?) -> Data? {
    ...
    func isMouseTrackingActive() -> Bool {
    ...
    func encodeMouse(action: GhosttyMouseAction, button: GhosttyMouseButton?,
    ...
    private(set) var currentSelection: GhosttySelection?
    ...
    func gridRefAtViewport(x: UInt16, y: UInt32) -> GhosttyGridRef? {
    ...
    func selectionPress(col: UInt16, row: UInt32, surfaceX: Double, surfaceY: Double, timeNs: UInt64) {
    ...
    func selectionDrag(col: UInt16, row: UInt32, surfaceX: Double, surfaceY: Double,
    ...
    func selectionRelease(col: UInt16, row: UInt32) {
    ...
    func selectionContains(col: UInt16, row: UInt32) -> Bool {
    ...
    func isBracketedPasteEnabled() -> Bool {
        guard let terminal else { return false }
        var enabled: Bool = false
        // GHOSTTY_MODE_BRACKETED_PASTE = ghostty_mode_new(2004, false) = 2004
    ...
    func hyperlinkAtViewport(col: UInt16, row: UInt32) -> String? {
    ...
    func setColors(fg: (UInt8, UInt8, UInt8), bg: (UInt8, UInt8, UInt8), cursor: (UInt8, UInt8, UInt8)) {
        guard let terminal else { return }
