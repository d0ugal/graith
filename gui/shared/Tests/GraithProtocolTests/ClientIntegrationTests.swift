...
struct ClientIntegrationTests {
    ...
    @Test func remoteHandshakeProofAndList() async throws {
        ...
        let server = Task {
            ...
            try await daemon.writeControl("handshake_ok", HandshakeOkMsg(version: "3.0", daemonVersion: "dev"))
    ...
    @Test func protocolClientRefreshesLocalHumanTokenPerConnection() async throws {
        ...
        let firstServer = Task {
            ...
            try await daemon.writeControl(
                "handshake_ok",
                HandshakeOkMsg(version: "3.0", daemonVersion: "dev")
            )
            let listReq = try await daemon.readControl()
            #expect(listReq.type == "list")
