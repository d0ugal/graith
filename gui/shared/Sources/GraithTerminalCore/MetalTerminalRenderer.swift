...
final class FontAtlas {
    ...
    /// Backing scale the atlas is rasterized at (device pixels per point).
    let scale: CGFloat
    /// Glyph cell size in atlas PIXELS (points × scale). The texture and the
    /// shader's texcoords work in these units, so glyphs are crisp at retina.
    let cellPxW: Int
    let cellPxH: Int
    ...
    init(font: PlatformFont, device: MTLDevice, scale: CGFloat = 1) {
        self.device = device
        ...
        // ceil (not round) so fractional scales never under-allocate the cell
        // by a pixel (avoids slight magnification / uneven sampling). [codex]
        self.cellPxW = max(1, Int(ceil(gw * self.scale)))
        self.cellPxH = max(1, Int(ceil(gh * self.scale)))
        ...
        let atlasPixelWidth = cellPxW * atlasCols
        let atlasPixelHeight = cellPxH * atlasRows
    ...
    private func renderGlyph(_ str: String, font: CTFont, slot: Int) {
        ...
        // Rasterize the point-size glyph into the scale× cell at device
        // resolution (crisp), instead of upscaling a 1× glyph.
...
public final class MetalTerminalRenderer: NSObject, MTKViewDelegate {
    ...
    private let atlasFont: PlatformFont
    ...
    public init(device: MTLDevice, font: PlatformFont = TerminalFontProvider.monospaced(ofSize: 13)) {
        ...
        self.atlasFont = font
        self.fontAtlas = FontAtlas(font: font, device: device, scale: backingScaleFactor)
    ...
    public func setBackingScaleFactor(_ scale: CGFloat) {
        guard scale > 0, scale != backingScaleFactor else { return }
        backingScaleFactor = scale
        // Rebuild the glyph atlas at the new device scale so text stays crisp
        // (cellWidth/Height/Descent are point-based and unchanged).
        fontAtlas = FontAtlas(font: atlasFont, device: device, scale: scale)
    }
    ...
    private func updateUniforms() {
        ...
        var cellUniforms = CellUniforms(
            ...
            glyphWidth: Float(fontAtlas.cellPxW),
            glyphHeight: Float(fontAtlas.cellPxH),
