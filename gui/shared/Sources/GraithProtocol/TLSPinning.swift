...
enum TLSPinning {
    ...
    static func leafMatchesSPKI(_ trust: SecTrust, expectedBase64: String) -> Bool {
        guard let leaf = leafCertificate(of: trust),
              let spkiHash = spkiSHA256(of: leaf) else {
            return false
        }
        return constantTimeEqual(spkiHash.base64EncodedString(), expectedBase64)
    }
    ...
    static func leafSPKIBase64(_ trust: SecTrust) -> String? {
        ...
        return spkiHash.base64EncodedString()
    }
    ...
    /// Compute SHA256 of the DER-encoded SubjectPublicKeyInfo.
    ///
    ...
    /// for the key's type+size — the standard iOS SPKI-pinning technique.
    private static func spkiSHA256(of certificate: SecCertificate) -> Data? {
