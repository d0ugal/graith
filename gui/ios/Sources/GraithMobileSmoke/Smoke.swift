...
func testHostRegistry() throws {
    ...
    check(registry.credentials(for: registry.host(id: "ben")!, deviceID: "d") == nil, "no creds before pairing")
    ...
    let creds = registry.credentials(for: registry.host(id: "ben")!, deviceID: "dev-bairn")
...
func testPairing() async throws {
    ...
    if case .paired = ok.phase { check(true, "successful pairing reaches .paired") }
    else { check(false, "successful pairing reaches .paired") }
    check(ok.spkiFingerprint != nil, "SPKI fingerprint surfaced for TOFU")
    check(identity.isPaired, "identity records the device id")
...
func testAppModel() async throws {
    ...
    let secrets = InMemorySecretStore()
    let identity = try DeviceIdentity(keychain: secrets)
    try identity.setDeviceID("dev-multi")
    let registry = HostRegistry(keychain: secrets, storeURL:
        FileManager.default.temporaryDirectory
            .appendingPathComponent("smoke-app-\(UUID().uuidString)/hosts.json"))

    // Two paired hosts: ben + brae.
    for id in ["ben", "brae"] {
