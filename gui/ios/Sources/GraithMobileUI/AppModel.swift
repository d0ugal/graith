...
public final class AppModel: ObservableObject {
    ...
    public func rebuildConnections() {
        ...
        connections = registry.hosts.compactMap { entry in
            ...
            guard let creds = registry.credentials(for: entry) else { return nil }
    ...
    /// "not on tailnet" when hosts exist but none connected — with no paired
    /// hosts there's nothing to infer from, so leave the current state alone.
    ...
    /// be dead. The client caches its control/event connections and reports
    /// `connected` for a cached-but-possibly-dead one, so a plain `connectAll()`
    /// would reuse the stale connection. Tear each connection down first so the
    /// reconnect dials a fresh one.
