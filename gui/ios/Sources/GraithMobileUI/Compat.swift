...
extension View {
    /// Use the compact inline navigation title on iOS (keeps the top bar small);
    /// a no-op on macOS where there's no large-title bar.
    @ViewBuilder
    func compactInlineTitle() -> some View {
        #if os(iOS)
