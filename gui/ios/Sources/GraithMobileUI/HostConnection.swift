...
public final class HostConnection: ObservableObject, Identifiable {
    ...
    private func startApprovalSubscription() {
        ...
        approvalTask = Task { [weak self] in
            ...
            // `approvalStream()` is actor-isolated on the client, so resolve it
            // inside the task. This Task inherits @MainActor, so assigning to
            // `approvals` below is main-actor safe.
