...
public final class TerminalAttachViewModel: ObservableObject {
    ...
    /// Geometry last *delivered to the remote PTY*. Tracked separately from the
    /// local grid because layout (and thus `resize`) usually runs before
    /// `attach()` sets `session` — without this the first size is recorded as
    /// "current" and every later identical resize short-circuits, so the remote
    /// PTY is stranded at its creation size and renders wider than the local
    /// grid (short-line + stray-char-at-right-edge garble). We re-push on attach.
    private var sentCols: UInt16 = 0
    private var sentRows: UInt16 = 0
    ...
    public func attach() async {
        ...
        do {
            ...
            // Layout usually ran before we had a session, so the view's real
            // size was never delivered. Push it now so the remote PTY matches
            // the local grid (otherwise it renders at its 80x24 creation size).
            if cols > 0, rows > 0 {
                sentCols = cols
                sentRows = rows
                let c = cols, r = rows
                Task { await session.resize(cols: c, rows: r) }
            }
        } catch {
            releaseClaim()
    ...
    public func resize(cols: UInt16, rows: UInt16, cellWidth: UInt32, cellHeight: UInt32) {
        ...
        // Forward to the remote PTY whenever it hasn't been told this size yet.
        // Gate on `sent*`, not `self.*`: a resize that arrives before `attach()`
        // updates the local grid but can't reach the remote, so the remote push
        // must still happen (here or on attach) once the session exists.
        guard let session, cols != sentCols || rows != sentRows else { return }
        sentCols = cols
        sentRows = rows
