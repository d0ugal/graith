...
# A cross-compile destination (not just -Xswiftc -target) so SPM treats the
# platform as iOS-simulator and selects the right libghostty-vt.xcframework
# slice (ios-arm64-simulator). -Xswiftc -target alone retargets the compiler but
# leaves SPM's binaryTarget slice selection on the host (macOS) → link failure.
DEST="$(mktemp "${TMPDIR:-/tmp}/ios-sim-dest.XXXXXX")"
BIN_DIR_TC="$(dirname "$(xcrun -f swiftc)")"
cat > "$DEST" <<JSON
{
  "version": 1,
  "sdk": "$SDK",
  "toolchain-bin-dir": "$BIN_DIR_TC",
  "target": "$TRIPLE",
...
BUILD_FLAGS=(--disable-sandbox --destination "$DEST" --product "$PRODUCT")
