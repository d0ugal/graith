...
| Trigger | `daemon/tracker.go` | Tracker action (#643): `gh` issue reader, pure `reconcileTracker` (spawn/resume/reap plan), `actionTracker` executor, grace-window state, reap policy |
...
| `tracker` | Poll an issue tracker and reconcile sessions against it (#643): each scheduled fire fetches active issues (GitHub Issues via `gh` in v1) and spawns one orchestrator-parented session per active issue (prompt seeded with `{issue_number}`/`{issue_title}`/`{issue_body}`/`{issue_url}`/`{issue_labels}`), reaping the session when its issue leaves the active state. Schedule source + `[orchestrator]` required. Config in `[trigger.action.tracker]` (`provider`/`repo`/`active_state`/`active_labels`/`assignee`/`grace`/`max_concurrent`/`reap`/`limit`). Dedup is by the durable `SessionState.TrackerIssue` tag (never respawns while a live session for the issue exists); a `grace` window guards against reaping on a brief inactivity; `reap` is `stop` (default) / `delete` (soft) / `none`. Read-only — never writes back to the tracker. Reconcile logic is the pure `reconcileTracker`; executor + `gh` reader in `daemon/tracker.go`. See `docs/design/2026-07-16-tracker-poll-action.md`. |
