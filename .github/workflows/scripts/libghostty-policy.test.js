...
test('Linux artifacts are lock-complete and published only by trusted immutable workflow', () => {
  ...
  assert.match(nativePublish, /sed "1i prefix=\\\$\{pcfiledir\}\/\.\."/);
  assert.match(nativePublish, /pkg-config --cflags libghostty-vt-static/);
  ...
  assert.match(nativeScript, /realpath.*include_path/);
  assert.match(nativeScript, /cflag_tokens\[\@\].*2/);
