...
flag is empty, doctor renders a temporary config with the same pure renderer as
`gr config alloy` and validates the temporary file.
...
`GRAITH_TEMPO_OTLP_ENDPOINT`) for common shape issues. Output names the
variable and sanitized shape only; it never prints userinfo, query strings,
fragments, usernames, tokens, or passwords.
...
and selected signals. That may be useful once Graith owns generated collector
files or managed user services, but it is premature for this local diagnostic
slice. Flags keep #2043 compatible with future lifecycle design without freezing
a config schema before managed collector ownership is decided.
...
- Managed collector lifecycle design:
  `docs/design/2026-08-04-managed-external-collector-lifecycle.md`
